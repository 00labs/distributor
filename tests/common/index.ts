...
export async function createAndFundWallet(connection: web3.Connection, keypair?: web3.Keypair) {
    if (!keypair) {
        keypair = web3.Keypair.generate();
    }

    const tx = await connection.requestAirdrop(keypair.publicKey, 1000 * web3.LAMPORTS_PER_SOL);
...
export const SET_COMPUTE_UNIT_LIMIT_IX = web3.ComputeBudgetProgram.setComputeUnitLimit({
    units: 1_400_000,
});
...
export const wrapSOL = async (connection: Connection, amount: BN, payer: Keypair) => {
    const ata = getAssociatedTokenAddressSync(NATIVE_MINT, payer.publicKey);
    const account = await connection.getAccountInfo(ata);

    if (!account) {
        await getOrCreateAssociatedTokenAccount(connection, payer, NATIVE_MINT, payer.publicKey);
    }
