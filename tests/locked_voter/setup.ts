...
import { IDL as LockedVoterIDL, LockedVoter } from "./locked_voter";
...
export function createLockedVoterProgram(wallet: Wallet): Program<LockedVoter> {
    ...
    return new Program<LockedVoter>(LockedVoterIDL, LOCKED_VOTER_PROGRAM_ID, provider);
}

export function deriveLocker(base: web3.PublicKey) {
...
export function deriveEscrow(locker: web3.PublicKey, owner: web3.PublicKey) {
    let [pk] = web3.PublicKey.findProgramAddressSync(
...
export async function createNewEscrowWithMaxLock(params: CreateNewEscrowParams) {
    ...
    await getOrCreateAssociatedTokenAccountWrap(
